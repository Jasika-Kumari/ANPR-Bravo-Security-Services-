...
db = client["license_plate_detection"]
    ...
    if len(license_detections.boxes.cls.tolist()) != 0:
        ...
        for license_plate in license_detections.boxes.data.tolist():
            ...
            if license_plate_text is not None and license_plate_text_score is not None:
                ...
                results[str(license_numbers)] = {  # Convert key to string here
                    'timestamp': timestamp,
                    'car': {
        ...
        # Save to JSON file
        with open("json_detections/detected_plates.json", "a") as json_file:
            json.dump(results, json_file, cls=JSONEncoder)
...
with header:
    ...
    st.write("The differents models detect the car and the license plate in a given image, then extracts the info about the license using EasyOCR, and crop and save the license plate as a Image, with a JSON file with all the data.")
...
with body:
    ...
    if colb2.button("Take a Photo", on_click=change_state_camera):
        pass
